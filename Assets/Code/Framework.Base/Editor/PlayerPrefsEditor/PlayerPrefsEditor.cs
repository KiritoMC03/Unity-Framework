...
using UnityEditor;
using UnityEngine;
using General.PlayerPrefsExtensions.Runtime;
using General.PlayerPrefsExtensions.Editor;
...
public class PlayerPrefsEditor : EditorWindow
{
    ...
    private bool automaticDecryption = false;
    ...
    private bool newEntryIsEncrypted = false;
    ...
    [MenuItem("Plugins/General Plugin/Player Prefs Editor")]
    ...
    private PlayerPrefPair[] RetrieveSavedPrefs(string companyName, string productName)
    {
        ...
        else if (Application.platform == RuntimePlatform.WindowsEditor)
        {
            ...
            Microsoft.Win32.RegistryKey registryKey =
                Microsoft.Win32.Registry.CurrentUser.OpenSubKey("Software\\Unity\\UnityEditor\\" + companyName + "\\" +
                                                                productName);
#else
            Microsoft.Win32.RegistryKey registryKey =
 Microsoft.Win32.Registry.CurrentUser.OpenSubKey("Software\\" + companyName + "\\" + productName);
    ...
    private void DrawAddEntry()
    {
        ...
        newEntryType =
            (PlayerPrefType)GUILayout.SelectionGrid((int)newEntryType, new string[] { "float", "int", "string" }, 3);
        EditorGUILayout.EndHorizontal();
            ...
            if (newEntryIsEncrypted)
            {
                ...
                if (newEntryType == PlayerPrefType.Float)
                    encryptedValue = PlayerPrefsUtility.VALUE_FLOAT_PREFIX +
                                     SimpleEncryption.EncryptFloat(newEntryValueFloat);
                else if (newEntryType == PlayerPrefType.Int)
                    encryptedValue = PlayerPrefsUtility.VALUE_INT_PREFIX +
                                     SimpleEncryption.EncryptInt(newEntryValueInt);
                else
                    encryptedValue = PlayerPrefsUtility.VALUE_STRING_PREFIX +
                                     SimpleEncryption.EncryptString(newEntryValueString);
    ...
    private void OnGUI()
    {
        ...
        {
            // Construct the plist filename from the project's settings
            string plistFilename = string.Format("unity.{0}.{1}.plist", PlayerSettings.companyName,
        ...
        else if (Application.platform == RuntimePlatform.WindowsEditor)
        {
            ...
            if (!lastDeserialization.HasValue ||
                DateTime.UtcNow - lastDeserialization.Value > TimeSpan.FromMilliseconds(500))
            {
