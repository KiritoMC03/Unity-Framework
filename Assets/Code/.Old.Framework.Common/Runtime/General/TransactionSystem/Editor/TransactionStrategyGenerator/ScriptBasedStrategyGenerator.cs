...
namespace GameKit.General.TransactionSystem.Editor
{
    public class ScriptBasedStrategyGenerator
    {
        ...
        {
            ScriptDescription script = GameKitUtils.FindScriptInUnityAssemblies(sourceScriptName);
            string fullPathToSourceScript = GameKitUtils.AssetPathToFullPath(script.path);
        ...
        {
            string fileTextStart = "using GameKit.General.TransactionSystem;\n\nnamespace " + targetNamespace + "\n{\n    public interface " + interfaceName + " : ITransaction\n    {";
            string fileTextEnd = "\n    }\n}";
